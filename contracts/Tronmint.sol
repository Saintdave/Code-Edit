...
import "./libraries/UDMath.sol";
import "./libraries/Utils.sol";

contract Tronmint is Utils {
    ...
    uint256 constant WITHDRAW_FEE = 12; // 10%
    ...
    uint256 internal tm_earn_balance;
    ...
    constructor() public {
        ...
        stake_percents[1] = 3e18; // 3% * MATH_BASE_UNIT
        stake_percents[2] = 55e17; // 5.5% * MATH_BASE_UNIT
        stake_percents[3] = 7e18; // 7% * MATH_BASE_UNIT
        stake_percents[4] = 105e17; // 10.5% * MATH_BASE_UNIT
    }
    ...
    /**
     * @dev Get Tronmint earn balance. 
     *
     * @return uint256. 
     */
    function getTMEarnBalance() 
        external 
        viwageTronmint
        return tm_earn_balance;
    }

    /**
     * @dev Get Tronmint current stake balance. 
     *
     * @return uint256. 
     */
        ...
        isAmountAvailable(amount) 
    {
        ...
        if (accounts[msg.sender].referred_by != address(0)) { // referred by user
            ...
            // add to Tronmine earn balance
            tm_earn_balance += stake_fee - referral_earn_amount;
            
        } else { // not referred by anyone
            // add to Tronmine earn balance
            tm_earn_balance += stake_fee;
        }
    ...
    function withdraw(uint256 amount) external isAmountAvailable(amount) {
        ...
        if (msg.sender.send(withdraw_amount)) { // transfer is successfully
            ...
            tm_earn_balance += withdraw_fee; // add to Tronmine balance
    ...
    /**
     * @dev Withdraw Tronmine earn balance. 
     *
     * @param amount Amount of TRX in sun. 
     */
    function withdrawTMEarn(uint256 amount) external onlyAdmin {
        // check if requested amount is available
        require(
            amount <= tm_earn_balance && amount > 0, 
